...
<!--/.Navbar -->
                      ...
                      <div class="col-8 contentask">
                        ...
                        <form class="border p-5" method="POST" enctype='multipart/form-data'>
                         ...
                         <!-- title -->
                         <label>Title*</label>
                         <input type="text" id="defaultContactFormName" class="form-control mb-4" placeholder="Question in general" name="title" required>
                         <!-- desciption -->
                         <label>Question*</label>
                         <div class="form-group">
                             <textarea class="form-control rounded-0" id="exampleFormControlTextarea2" rows="3" placeholder="Explain more" name="content" required></textarea>
                         </div>
                         <!-- Code -->
                         <label>Code</label>
                         <div class="form-group">
                             <textarea class="form-control rounded-0" id="exampleFormControlTextarea2" rows="3" placeholder="Your code here" name="code"></textarea>
                         </div>
                         <!-- add button -->
                         <center>
                           <input class="btn btn-primary btn-md rounded-pill" name="ask" type="submit" value="Ask !">
                         </center>
...
if (isset($_POST['ask'])){
    ...
    $post->topic = stripslashes($_REQUEST['topic']);
    $post->topic = mysqli_real_escape_string($conn, $post->topic);
    ...
    $query = "INSERT into post (title, content, code,  date, report , user_id, topic_id)
              VALUES ('$post->title', '$post->content', '$post->code', '$post->date','$post->report', '$post->userid','$post->topic')";
